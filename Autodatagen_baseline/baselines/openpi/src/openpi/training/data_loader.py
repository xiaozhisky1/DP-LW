...
def create_torch_dataset(
    ...
    # Support local dataset root
    local_root = data_config.local_dataset_root
    try:
        dataset_meta = lerobot_dataset.LeRobotDatasetMetadata(repo_id, root=local_root)
    except Exception as e:
        if local_root is None:
            error_msg = (
                f"Failed to load dataset '{repo_id}' from HuggingFace. "
                f"This may be due to version compatibility issues. "
                f"Error: {e}\n\n"
                f"Solutions:\n"
                f"1. Update lerobot: pip install --upgrade lerobot\n"
                f"2. Use a local dataset by setting 'local_dataset_root' in your DataConfig\n"
                f"3. If you have the dataset locally, set it in your config like:\n"
                f"   data=LeRobotX7sDataConfig(\n"
                f"       repo_id='{repo_id}',\n"
                f"       local_dataset_root='/path/to/your/local/dataset'\n"
                f"   )"
            )
...
def create_behavior_data_loader(
    ...
    *,
    ...
    if data_config.behavior_dataset_root:
        # Use BehaviorLeRobotDataset for B1K datasets
        dataset = create_behavior_dataset(data_config, action_horizon=config.model.action_horizon)
    elif data_config.local_dataset_root:
        # Use standard LeRobotDataset for local datasets (like X7S)
        dataset = create_torch_dataset(data_config, config.model.action_horizon, config.model)
    else:
        # Default to behavior dataset for backward compatibility
        dataset = create_behavior_dataset(data_config, action_horizon=config.model.action_horizon)
